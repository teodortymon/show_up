...
Template.days.helpers({
	...
	allEvents: function () {
		...
		if(Roles.userIsInRole(Meteor.user()._id, 'admin') || Roles.userIsInRole(Meteor.user()._id, 'sectionAdmin') )
...
Template.oneEvent.helpers({
  ...
  isEventFull: function() { 
    ...
    console.log(this.users.length +' '+this.limit);
...
Template.adminEvent.events({
  ...
  "submit #manually-add": function (event) {
    ...
    console.log(this);
    Meteor.call("userManuallyAdd",Meteor.users.findOne({ 'emails': { $elemMatch: { 'address': event.target.text.value} } } )._id, this);
